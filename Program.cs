...
namespace backEnd
{
    public class DatabaseManager : IDisposable
    {
        ...
        public struct Item 
        {
            ...
            public int? IncomeRemovedId { get; set; }

            // Constructor to ensure all fields are initialized
            public Item(int itemId, string itemName, long price, bool oneTime, bool inInventory, bool intermediary, string? command, int? incomeSourceId, int? incomeRemovedId)
            {
                ItemId = itemId;
                ItemName = itemName;
                Price = price;
                ...
                IncomeRemovedId = incomeRemovedId;
            }
        }
        ...
        {
            {
                ...
                try
                {
                    transaction = _serverConnection.BeginTransaction(); // Start the transactionnamespace
        ...
        public async Task<bool> InitializeItem(
            ...
            int incomeRemovedId = -1,
            string command = "")
        {
            EnsureConnectionOpen(); 
            try
            {
                string insertSql = "INSERT INTO master_items (one_time, income_source_id, command, item_name, price, in_inventory, intermediary, income_removed_id) " +
                                   "VALUES (@oneTime, @incomeSourceId, @command, @itemName, @price, @inInventory, @intermediary, @incomeRemovedId)";
                ...
                using (MySqlCommand cmd = new MySqlCommand(insertSql, _serverConnection))
                {
                    ...
                    cmd.Parameters.AddWithValue("@intermediary", intermediary);

                    // Handle nullable incomeSourceId: if it's -1 (your default for null), pass DBNull.Value
                    if (incomeSourceId == -1) { cmd.Parameters.AddWithValue("@incomeSourceId", DBNull.Value); }
                    else { cmd.Parameters.AddWithValue("@incomeSourceId", incomeSourceId); }

                    // Handle nullable command: 
                    if (string.IsNullOrEmpty(command)) { cmd.Parameters.AddWithValue("@command", DBNull.Value); }
                    else { cmd.Parameters.AddWithValue("@command", command); }
                   
                    // Handle nullable incomeRemovedId
                    if (incomeSourceId == -1) { cmd.Parameters.AddWithValue("@incomeRemovedId", DBNull.Value); }
                    else { cmd.Parameters.AddWithValue("@incomeRemovedId", incomeRemovedId); }
        ...
        public async Task<bool> AddItemToUser(string discorduserId, int masterItemId, string serverId) // Renamed for clarity
        {
            ...
            try
            {
                ...
                // If it only exists to add the income source
                if (itemDetails.Intermediary) { return true; }
        ...
        public async Task<Item> ReadItem(int itemId) // Return Item? to indicate it might be null
        {
                ...
                using (MySqlCommand cmd = new MySqlCommand(sql, _serverConnection))
                {
                    ...
                    using (MySqlDataReader reader = (MySqlDataReader)await cmd.ExecuteReaderAsync())
                    {
                        if (reader.Read())
                        {
                            ...
                            int? incomeRemovedId;
                            int incomeRemovedIdDatabaseIndex = 7;
                            if ( reader.IsDBNull(incomeRemovedIdDatabaseIndex) ) { incomeRemovedId = null; }
                            else { incomeRemovedId = reader.GetInt32("income_removed_id"); }
                            ...
                            return new Item(
                                ...
                                intermediary: reader.GetBoolean("intermediary"),
                                command: command,
                                incomeSourceId: incomeSourceId,
                                incomeRemovedId: incomeRemovedId
                            );
            ...
            {
                Console.WriteLine($"Error reading item {itemId} from master_items, index out of range");
...
namespace economyBot
{
    class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            backEnd.DatabaseManager dbManager = new backEnd.DatabaseManager();
            ...
            // Add in money
            await dbManager.AddCashToUserAsync("1234", "1234", 100, backEnd.DatabaseManager.TransactionType.income, "test");
            await dbManager.AddBankToUserAsync("1234", "1234", 1000, backEnd.DatabaseManager.TransactionType.sell, "test");
